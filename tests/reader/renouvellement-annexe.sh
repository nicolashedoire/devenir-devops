...
# SHA-256 de leur concaténation : 48b5abaa377fa055bf753c0a16b1217efbb059015a45f6fabfb94811760364ad
set -e
umask 077
...
TASKBOARD_TLS_STAGE=$(mktemp -d \
  "$TASKBOARD_TLS_SOURCE/renewal.XXXXXX")
cp "$TASKBOARD_TLS_SOURCE/ca.crt" \
  "$TASKBOARD_TLS_STAGE/ca-avant.crt"
cp "$TASKBOARD_TLS_SOURCE/runtime/server.crt" \
...
cmp "$TASKBOARD_TLS_STAGE/ca-avant.pub" \
  "$TASKBOARD_TLS_STAGE/ca-apres.pub"
openssl verify -purpose sslserver -verify_hostname localhost \
  -CAfile "$TASKBOARD_TLS_STAGE/ca.crt" \
  "$TASKBOARD_TLS_STAGE/server.crt"
...
install -m 600 "$TASKBOARD_TLS_STAGE/ca.crt" \
  "$TASKBOARD_TLS_SOURCE/ca.crt"
